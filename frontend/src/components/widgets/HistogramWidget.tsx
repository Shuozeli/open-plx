...
export function HistogramWidget({ config, data, loading, error, onClickInteraction }: WidgetProps) {
  ...
  useEffect(() => {
    registerWidget({
      ...
      rendered: {
        ...
        encodeX: encode?.x ?? null,
        hasBinTransform: Array.isArray(g2Spec?.transform) &&
          (g2Spec.transform as Record<string, unknown>[]).some((t) => t.type === "binX"),
      },
      updatedAt: Date.now(),
    });
  }, [config, data, chartSpec, g2Spec, encode]);
