...
import { wordCloudProtoToG2, wordCloudProtoToTestState } from "../../services/mappers/wordCloudMapper.js";
...
export function WordCloudWidget({ config, data, loading, error, onClickInteraction }: WidgetProps) {
  const spec = config.spec?.spec.case === "wordCloud" ? config.spec.spec.value : null;
  const g2Spec = spec && data ? wordCloudProtoToG2(spec, data) : null;
  ...
  useEffect(() => {
    registerWidget({
      ...
      rendered: {
        ...
        ...(spec && data ? wordCloudProtoToTestState(spec, data) : {}),
      },
      updatedAt: Date.now(),
    });
