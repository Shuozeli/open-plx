...
export function ScatterChartWidget({ config, data, loading, error, onClickInteraction }: WidgetProps) {
  ...
  useEffect(() => {
    registerWidget({
      ...
      rendered: {
        ...
        encodeY: encode?.y ?? null,
        encodeSize: encode?.size ?? null,
        encodeColor: encode?.color ?? null,
      },
      updatedAt: Date.now(),
    });
