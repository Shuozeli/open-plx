...
import { funnelProtoToG2, funnelProtoToTestState } from "../../services/mappers/funnelMapper.js";
...
export function FunnelWidget({ config, data, loading, error, onClickInteraction }: WidgetProps) {
  const spec = config.spec?.spec.case === "funnel" ? config.spec.spec.value : null;
  const g2Spec = spec && data ? funnelProtoToG2(spec, data) : null;
  ...
  useEffect(() => {
    registerWidget({
      ...
      rendered: {
        ...
        ...(spec && data ? funnelProtoToTestState(spec, data) : {}),
      },
      updatedAt: Date.now(),
    });
