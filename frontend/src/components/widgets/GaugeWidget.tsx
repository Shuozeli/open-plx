...
import { gaugeProtoToG2, gaugeProtoToTestState } from "../../services/mappers/gaugeMapper.js";
...
export function GaugeWidget({ config, data, loading, error, onClickInteraction }: WidgetProps) {
  const spec = config.spec?.spec.case === "gauge" ? config.spec.spec.value : null;
  const g2Spec = spec && data ? gaugeProtoToG2(spec, data) : null;
  ...
  useEffect(() => {
    registerWidget({
      ...
      rendered: {
        ...
        ...(spec && data ? gaugeProtoToTestState(spec, data) : {}),
      },
      updatedAt: Date.now(),
    });
